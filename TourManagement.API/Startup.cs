...
namespace TourManagement.API
{
    public class Startup
    {
        ...
        public void ConfigureServices(IServiceCollection services)
        {
            ...
            {
                option.AddPolicy("UserMustBeAdministrator", policy =>
                {
                    policy.RequireAuthenticatedUser();
                    policy.RequireRole("Administrator");
                });

                option.AddPolicy("UserMustBeTourManager", policy =>
                {
                    policy.RequireAuthenticatedUser();
                    policy.AddRequirements(new UserMustBeTourManagerRequirement("Administrator"));
                });
            });
        ...
        public void Configure(IApplicationBuilder app, IHostingEnvironment env)
        {
            ...
            AutoMapper.Mapper.Initialize(config =>
            {
                ...
                config.CreateMap<Dtos.TourForCreation, Entities.Tour>().ReverseMap();
                config.CreateMap<Dtos.TourWithManagerForCreation, Entities.Tour>().ReverseMap();

                config.CreateMap<Entities.Tour, Dtos.TourWithShows>()
                .ForMember(d => d.Band, o => o.MapFrom(t => t.Band.Name));

                config.CreateMap<Entities.Tour, Dtos.TourWithEstimatedProfitsAndShows>()
                .ForMember(d => d.Band, o => o.MapFrom(s => s.Band.Name));

                config.CreateMap<Dtos.TourWithShowsForCreation, Entities.Tour>().ReverseMap();
                config.CreateMap<Dtos.TourWithManagerAndShowsForCreation, Entities.Tour>().ReverseMap();
                config.CreateMap<Dtos.ShowForCreation, Entities.Show>().ReverseMap();
