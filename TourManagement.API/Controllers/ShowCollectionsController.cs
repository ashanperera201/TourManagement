...
namespace TourManagement.API.Controllers
{
    ...
    public class ShowCollectionsController : Controller
    {
        ...
        [HttpPost]
        [RequestHeaderMatchesMediaType("Content-Type", new[] { "application/vnd.marvin.showcollectionforcreation+json" })]
        public async Task<IActionResult> CreateShowCollection(Guid tourId, [FromBody] IEnumerable<ShowForCreation> showCollection)
        {
            ...
            if (!await _tourManagementRepository.SaveAsync()) { throw new Exception("Error"); }
            return Ok();
        }
